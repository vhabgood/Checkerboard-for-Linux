...
inline int lsb_(uint32_t x)
{
	return bit_scan_forward(x);
}


inline int msb_(uint32_t x)
{
...
#ifdef ENVIRONMENT64
inline int lsb64_(uint64_t x)
{
	return bit_scan_forward64(x);
}

inline int msb64_(uint64_t x)
{
...
{
	if (x & 0xffffffff)
		return(lsb_((uint32_t)(x & 0xffffffff)));
	else
		return(32 + lsb_((uint32_t)(x >> 32)));
}

inline int msb64_(uint64_t x)
{
	if (x & 0xffffffff00000000)
		return(32 + msb_((uint32_t)(x >> 32)));
	else
		return(msb_((uint32_t)(x & 0xffffffff)));
}
#endif

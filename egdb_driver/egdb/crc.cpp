...
unsigned int crc_calc(const char *buf, int len) {
    int i;
    unsigned int crc = CRCSEED;
    for (i = 0; i < len; ++i) crc = CRC32tab[(crc ^ buf[i]) & 0xff] ^ (crc >> 8);
    crc ^= XOROT;
    return(crc);
}
...
unsigned int file_crc_calc(FILE *fp, int *abort) {
    ...
    do {
        ...
        for (i = 0; i < bytes_read; ++i) crc = CRC32tab[(crc ^ buf[i]) & 0xff] ^ (crc >> 8);
    } while (bytes_read > 0);
    crc ^= XOROT;
    return(crc);
}
...
int fname_crc_calc(const char *name, unsigned int *crc) {
    ...
    if (!fp) return(1);
    int abort_flag = 0; // Not used currently
    *crc = file_crc_calc(fp, &abort_flag);
    fclose(fp);
    return(0);
}

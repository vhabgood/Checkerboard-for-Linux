...
namespace egdb_interface {
	...
	EGDB_ERR egdb_close_wld_tun_v2(EGDB_DRIVER *driver)
	{
		if (!driver)
	...
	unsigned int egdb_get_max_pieces_wld_tun_v2(EGDB_DRIVER *driver)
	{
		return(driver->max_pieces);
	}

	unsigned int egdb_get_info_wld_tun_v2(
		EGDB_DRIVER *driver,
	...
	int egdb_wld_tun_v2_lookup(EGDB_DRIVER *driver, const EGDB_POSITION *pos, EGDB_ERR *err_code)
	{
		DBHANDLE h;
		int value;
		...
		if (*err_code != EGDB_NORMAL)
			return(EGDB_UNKNOWN);

		return(value);
	}

	EGDB_ERR egdb_open_wld_tun_v2(EGDB_DRIVER *driver)
	{
		DBHANDLE h;
	...
	*/
	static DBHANDLE initdblookup(char const *egtb_dir, unsigned int cache_size_bytes)
	{
		...
		h = (DBHANDLE)std::calloc(1, sizeof(struct DB_HANDLE_T));
		if (!h)
			return(0);
		
		h->db_type = EGDB_WLD_TUN_V2;
		h->max_pieces = 0;
		h->compression_type = EGDB_COMPRESSION_TUNSTALL_V2;

		// allocate the lru cache memory.
		h->cache_base = (uint8_t *)std::calloc(cache_size_bytes, 1);
		if (!h->cache_base) {
			std::free(h);
			return(0);
		}
		...
		h->cache_block_ptr = (uint8_t **)std::calloc(MAXCACHEDBLOCKS, sizeof(uint8_t *));
		...
		h->cache_block_info = (BLOCK_INFO *)std::calloc(MAXCACHEDBLOCKS, sizeof(BLOCK_INFO));
	...
	static unsigned int parseindexfile(DBHANDLE h, char const *idx_fname, int file_num)
	{
		...
		for (i = 0; i < num_lines; ++i) {
			...
			idx_rec = (INDEX_REC *)std::calloc(1, sizeof(INDEX_REC));
			if (!idx_rec) {
				std::fclose(fp);
				return(1);
			}
	...
	*/
	static int dblookup(DBHANDLE h, const EGDB_POSITION *pos, EGDB_ERR *err)
	{
		...
		*err = EGDB_ERR_NORMAL;
		...
		// These dbs are split by piece count (6, 7 and 8 pieces).
		// We have to get the lsb_index and msb_index from the index.
		lsb_index = lsb64_(index);
		msb_index = msb64_(index);
		decompress_wld_tunstall_v2(db_data_block, db_len_bytes,
